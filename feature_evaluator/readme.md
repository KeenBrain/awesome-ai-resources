...
A step-by-step prompt guide for product managers.

---

## What You’re Building

A browser-based tool that helps product teams prioritize features using a 2x2 matrix. You score each feature on two dimensions:
...
That’s it. No images, no existing code repos, no reference files. The prompts below describe everything Claude needs in plain text. You don’t need to know HTML, CSS, JavaScript, or any programming language. Claude writes all the code. You just tell it what you want.
...
**Why this prompt works:** You’re giving Claude three things it needs upfront: the visual layout (a 2x2 grid with labeled quadrants), the interaction model (scoring controls), and the technical constraints (a single HTML file and Tailwind CSS). Being specific about the quadrant names and descriptions means the AI doesn’t have to guess at your framework.
...
**Why this prompt works:** The first version may place the input controls on the left and the grid on the right. That’s a common default layout, but it buries the most important element—the grid itself—in a two-column view. By moving controls underneath, you’re making the grid the hero of the page. Product managers will spend most of their time looking at the grid, not the sliders, so it should dominate the screen.
...
This is the same principle used in agile story point estimation. It forces honest prioritization by making the cost of over-scoring feel real.

**What to expect:** Claude may replace the 1–10 sliders with a 6-step Fibonacci scale. Each slider snaps to discrete values (1, 2, 3, 5, 8, 13) with tick marks underneath. The quadrant boundary will be set at the 3/5 threshold — features scored 5 or above on an axis land in the “high” zone for that dimension. Save and refresh.
...
No server, no database, no login. The data lives in the URL itself. The only requirement is that your colleague has the same HTML file—just send it along with the link or host it on an internal web server.
